...
- You can finish a build you do not have all the parts for. The assembly is
  ...
  actually paid, and says so in the activity feed; producing build output does
  the same for the assembly it builds. If the stock is already on the shelf
  instead, **Consolidate stock** on the part page clears every outstanding
  shortfall for that part in one click (oldest debt first), and **Settle from
  stock** on a negative item's page pays one off by hand: it asks how many, and
  which stock to take them from (oldest first by default, like a stocktake).
  A settled shortfall and a
...
- A stock shortage report answering "what do I need to buy?": actual stock minus
  what the open sales orders need, for every part, with every assembly that comes
  out short exploded through its BOM (only the shortfall -- the units on the shelf
  cover that many sales), recursively, so the list is only parts you can actually
